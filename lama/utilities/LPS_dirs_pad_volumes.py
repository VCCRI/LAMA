...
$ LPS_pad_volumes.py -i dir1 dir2 --clobber

# To pad a series of volumes and write to new directory
$ LPS_pad_volumes.py -i dir1 dir2 --outdir new_dir

# To pad a series of volumes to a defined shape (xyz)
$ LPS_pad_volumes.py -i dir1 dir2 --outdir new_dir -d 100 200 300
...
def makeLPS(indirs: Iterable[Path], outdir: Path, clobber: bool, filetype: str='nrrd'):
    #
    if clobber and outdir:
        print('Specify either --clobber or an output dir (-o)')
        return
    ...
    for dir_ in indirs:
        ...
        for path in volpaths:
            ...
            # loop over all label images in the folder. Make sure nothing else is in there
            
            print(f'doing {path}')
            img, head = nrrd.read(path)
            print(head)
            print(type(head))
            #head['encoding'] = 'gzip' # change value of 'encoding' key to gzip from raw, so that the NRRD output is compressed
            #head['space'] = 'left-posterior-superior'
            #print(head)
            nrrd.write(str(outpath), img, header=options) # saves NRRD file 
        print("finished change to LPS")
...
def pad_volumes(indirs: Iterable[Path], max_dims: Tuple, outdir: Path, clobber: bool, filetype: str='nrrd'):
    ...
    for dir_ in indirs:
        ...
        for path in volpaths:
            ...
            sitk.WriteImage(padded_vol, str(outpath), useCompression = True) # set to save NRRDs compressed
            #https://discourse.itk.org/t/large-size-when-saving-image-using-simpleitk-writeimage/3509
...
def main():
    ...
    makeLPS(indirs, outdir, args.clobber) #probably needs adjusting
